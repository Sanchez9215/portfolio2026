...
**When a build detail is unknown (e.g. where a new component gets placed), ask the user directly — never go spelunking through an external source (a large Figma file's other pages, an unrelated part of the codebase) trying to reverse-engineer the answer.**
This happened for real: asked where a new Banner component belonged on the screen, and instead of asking, went digging through a Figma file's other top-level pages and a large, unrelated frame tree to guess the placement from context clues — burning two tool calls (one dumping an 84k-character file to disk) on an exploration nobody requested, for a question one sentence to the user would have answered immediately. "Ask before acting" (above) already covers this, but the specific failure mode — treating an unknown detail as a research problem to solve alone rather than a question to ask — is common enough to name directly. If the answer isn't already established in the current task's own context (the file being edited, the component just discussed), ask; don't go looking for it.

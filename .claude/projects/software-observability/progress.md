...
### `testing-the-experience` — live connected embed + ghost-cursor autoplay (phase 1 + 2)
Superseded the static-image plan — the "Full Prototype" ImgCard now holds `LegacyExperienceEmbed` (real `OverviewScreen` ⇄ legacy `AllSoftwareLegacy` → `SoftwareProfileLegacy`, no hotspots) plus `GhostCursor`'s scripted autoplay walkthrough. See `built-components.md` for both. Confirmed working this session (verified via captured DOM: full run completes, bubble types in full, ends frozen/faded at 0 opacity) — trigger point (embed top vs `--nav-height`) and interaction lock (blocks real clicks/scroll until a real click) both fixed after two rounds of live debugging.

**Known open items:**
- Script only fires once per page load (`armed` never resets) — needs a fresh load to replay, no re-arm on scroll-away-and-back.
- Unrelated pre-existing console errors surfaced during this session's debugging: GSAP `<line>` attribute errors (`x1`/`y1`/`x2`/`y2` "Unexpected end of attribute") from some other SVG line animation on the page (likely `FrameworkScene`'s connectors) — not touched, flagged but not investigated.
- Ghost-cursor script (waypoints/timing) was drafted by the agent and approved in principle, not pixel-tuned — revisit hover/hold durations and the exact beat order if it reads off during a full watch-through.
- Deferred, not built: an exit/restart control for the 4 existing hotspot embeds (Overview 1/2, All Software, Profile) so a visitor who navigates out mid-sequence (e.g. All Software's row-click still opens the real Profile panel during the pinned hotspot flow) can back out and replay — user flagged this as real but explicitly lower priority than this section.

...
- **Screen shell** — `app/work/data-health-monitor/` (page/layout/`DataHealthScreen.tsx`), `Sidebar` + `GlobalHeader` reused as-is
...
- **Domain Specific view** (Figma 709:600, `DomainHealthScreen.tsx`, route `/work/data-health-monitor/domain`) — Infrastructure domain only, reached by clicking the Health by Domain table's Infrastructure row (other domains inert until built)
...
- **`ProgressBar`'s 164px min-width** overshoots Figma's 111px bars in Open Remediation Requests/its nested department table — accepted deviation, not fought.

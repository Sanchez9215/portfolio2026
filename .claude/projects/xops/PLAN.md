...
13. **[Low priority] `SidePanel` always-mounted fixed overlay** — `SidePanel`'s outer `.overlay` (`design-systems/xops/components/SidePanel.module.css`) is `position: fixed` and is always rendered, even when closed (just slid off-screen via `transform: translateX(100%)`), rather than conditionally rendered (`{isOpen && <SidePanel/>}`). Surfaced by a software-observability session where this caused a rounded-corner clipping bug in a live embed (an always-mounted fixed overlay bleeding past an ancestor's `overflow:hidden` — worked around there without touching this component). Architectural cleanup, not a correctness bug in SidePanel's own usage today.

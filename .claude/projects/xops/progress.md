...
- **banner** — `Banner.tsx`/`.module.css` (Figma 666:559), persistent tinted disclosure; `danger` live in both All Software (org-wide, inside `main` above `PageHeader`, "X Licenses Over-Assigned across Y Products") and Software Profile (`headerSection`, per-product); `risk`/`info`/`neutral`/`success` scaffolded, unused; optional `style` passthrough for one-off layout overrides
- **indicator** — `Indicator.tsx`, `icon` variant only (`dot` throws — unbuilt, no confirmed size yet); wraps `Icon` at `size="18"`, status color off the shared `TagStatus` enum
- **icon `size` prop** — `Icon.tsx` gained `size?: "16"|"18"|"20"` (default `"20"`); existing per-consumer hand-rolled overrides not migrated (see Deferred)
- **All Software Assigned column** — new column (`Opportunity, Assigned, Utilization, ...`), `count (percent%)` via `formatCountWithPercent`; danger text + `Indicator` warning icon (fixed-left, decoupled from text width) only when `excessAssigned > 0`; consumption rows show `—`
- **Renewal Manual/Automatic + Overdue** — `generate.ts`: only `Manual` contracts can lapse into negative `renewalDays` (`Automatic` renews itself, never past due); `formatRenewalDuration` abbreviates (`32d`/`3mo`/`2yr`), negative days render `"Xd Overdue"`
- **Renewal date column alignment** — fixed `12ch` right-aligned date slot + `tabular-nums`, decouples the urgency Tag's start position from date string width
- **status.danger.tint → danger.100** (was danger.200) — affects `Banner` and `Tag`'s danger background together
- **font-weight.semibold** (600) — new primitive, first use: Banner title

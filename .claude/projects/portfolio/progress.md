...
### Overview Prototype 1/2 — unified card player + Timeline sidebar (multi-session, ongoing — blind pass throughout)
Card player is one sequence: intent → insights (same cards, `showInsight` text) → switching (embed swaps to Prototype 2, hold) → decisions (migrated from the now dead/reference-only `OverviewPrototype2Hotspots.tsx`). `AnnotationConnectorHotspot.tsx` has a sequential fade transition (200ms out/300ms in) plus `targetSelectors`/`widthFromSelector`/`documentScoped` targeting for Decisions' harder cases. Countdown starts at 8s (was 3).

**Sidebar redesign, driven by Figma nodes 941:774/941:812/941:810 — building section by section, aligned live with the user before each one; eventual direction is a shared reusable "Player" component for the other (currently `HotspotOverlay`-based) hotspot experiences, but deliberately deferred until this Overview 1/2 instance is validated in-browser first — build as a normal component now.**

**Section 1 (done this session): countdown + button + keyboard-shortcuts panel, moved from `ImgCard`'s embed header into `Timeline.tsx`'s new top "player" block.**
- **State lifted:** `OverviewPrototypeHotspots.tsx` remains the sole owner of `countdown`/`exited`/`phase`/`cardStep` — it reports a derived `PlayerState` (`{countdown, exited, started, onStartNow, onExitWalkthrough, onStartWalkthrough}`, new type exported from `Timeline.tsx`) up to `OverviewPrototypeSection.tsx` via a new `onPlayerStateChange` callback, same relationship `onMilestoneChange`/`activeMilestone` already had. `ImgCard` no longer has any countdown/exited/button props at all — its embed header is just caption + progress track now.
- **Button slot:** one shared full-width slot swapping Start Now ⇄ Exit ⇄ Restart (fade only, via a small `FadeSwap` helper — no width tween needed since these are full-width, unlike the old content-hugging header buttons). Went through several iterations before settling: tried Figma's exact primary/medium flat treatment, reverted to the pre-existing secondary + `headerButtonIcon`-style treatment (16px label, 8px gap, symmetric padding) per user preference, then `size="xsmall"`(32px)→`size="m"`(40px) per follow-up. **Extended the `--button-label-size/-lh/-ls` var-with-fallback pattern (first added to `.xsmall .label`) to `.m .label` too** in `Button.module.css` — needed it again when the size changed.
- **Countdown fade-down:** now fades opacity+`yPercent` (0→100, 0.185s) once the walkthrough starts or exits, instead of a hard unmount — element stays mounted permanently once a real countdown value has ever shown (`displayCountdown` state freezes the last value so the exit tween has something to animate), visibility driven by GSAP, matching the same "always mounted, GSAP controls visibility" convention `FadeSwap` in this file already uses.
- **Shortcuts panel** (Play/Pause → "Space Bar" pill; Tooltip Navigation → left/right arrow icon pills) is purely informative, no new interaction wired — documents controls that already work. Uses `public/icons/arrow_left.svg`/`arrow_right.svg` (user pre-exported these from Figma this session).
- **New `.topGroup`** wraps the player block + the existing Timeline heading/rows block at a 32px gap, matching Figma's real grouping (`1004:425`) — sets up cleanly for that node's border-bottom divider, which is NOT built yet (belongs to the next section).
- **Cross-module CSS specificity bug pattern, hit repeatedly this session — remember for next time:** CSS Modules hash class names per file, so a bare `.label`/`.xsmall` selector written in a *different* module (e.g. `Timeline.module.css` trying to reach `Button.module.css`'s real `.label`/`.xsmall` classes) silently does nothing — looks like it should work, doesn't. Fixed via CSS custom properties (which inherit through the DOM regardless of module scoping) for typography overrides, and an element-qualifier (`button.foo` vs `.foo`) for specificity fights against `.btn`'s own base rules.
- **Token gap resolved:** Figma wanted a 2px radius on the shortcut pills; no 2px token exists, used `--border-radius-xs` (4px) per user confirmation.

**Since Section 1:** divider added under the Timeline rows block (`.topGroup` border-bottom + `.color-grey-1000`), "Skip to... / Prototype 02" label section built (node 941:836), per-section 16px padding replacing one blanket `.timeline` padding-left, several copy/token fixes (heading "Timeline" not "Walkthrough Timeline", "Design Intent"/"Learnings" row labels, 12px hint/shortcut text, new `--spacing-sm-md` 12px token for row padding, active-step background highlight). Button content crossfade reworked so the `<Button>` itself never duplicates — only `PlayerButtonContent` (icon+label) swaps inside it.

**Next session starts here — mid-design, nothing built yet:** exploring a "double button set" redesign of the player controls (primary+secondary pair per state: Play Now/Skip while counting down, Pause⇄Play/Leave once started, Replay alone once exited — Leave likely drops in the Replay state since it's a no-op once already exited, pending final confirm). Found 3 real edge cases in `OverviewPrototypeHotspots.tsx` that any implementation must handle: (1) Skip must also clear the countdown (`setCountdown(null)`) or the existing auto-kickoff effect will un-skip the user when it hits 0; (2) `paused` is never reset in `handleStartWalkthrough`, so Replay after a pause restarts already-frozen; (3) confirm whether "Play Now" should get an icon like Pause/Replay do (mockup shows it bare). `paused` state + its toggle already exist in the component (wired to Space) but aren't yet exposed via `PlayerState` — needs a `paused`/`onTogglePause` field added. Escape-to-Leave also agreed (scoped to when Leave is visible, added to the shortcuts panel) — user has designs for the shortcut-panel addition, not yet shared.
...
- `WorkCaseStudyRow`'s large ghost cursor + `MessageBubble` stack (Figma node 796:930) — **removed entirely this session** (was "toggled off, kept for reuse" per the prior entry above; user decided against bringing it back, code deleted, not just disabled). `GhostCursor.tsx`/`MessageBubble.tsx` themselves are untouched and still used elsewhere (`SoftwareExperienceEmbed`'s own small cursor).
...
### Home — Resume Context
- **`WorkCaseStudyRow` generalized to all 4 case studies + video/live embeds wired in — nothing below has had a full visual confirmation pass.** `WorkCaseStudyRow.tsx` (was SW-Obs-only, hardcoded to `introContent.ts`/`SoftwareExperienceEmbed`) now takes a `CaseStudyIntro` + a `visual` render prop (`(entranceReady, settled) => ReactNode`); `WorkSection.tsx` (new) composes all 4 rows and lives in `app/page.tsx`'s place — `app/page.tsx` stays a Server Component since the render-prop composition needs `"use client"`. Order: Software Observability (live `SoftwareExperienceEmbed`, unchanged) → Data Health Monitor (new `DataHealthEmbed.tsx` — the real `DataHealthScreen` in `LiveEmbed`'s `scroll` mode, non-interactive via `pointer-events:none`, no scripted walkthrough, "first page only" per user) → Session Replay (`SessionReplayVideo.tsx`, plays `/videos/SessionReplay_Final.mp4`) → Path Analysis (`PathAnalysisVideo.tsx`, plays `/videos/PathAnalysis_Final.mp4`). Both videos gate playback on `settled` (fires when the row's own entrance tween *finishes*, not starts — new second arg on `visual`, distinct from the pre-existing `entranceReady`) and loop.
  - `.embedWrap`'s default flipped: hugs the visual's own real height (all 4 corners round) instead of a forced 16/10 box. New `fixedVisualRatio` prop opts a row into the old fixed-16/10-with-scroll-window behavior (SW Obs and DHM both use it — DHM's live embed needs the same bounded scroll viewport SW Obs's does). New separate `flushBottomRadius` prop controls top-only rounding independently of the ratio (SW Obs only — was bundled into the ratio class at first, split apart after DHM inherited SW Obs's square-bottom look by accident and it wasn't wanted there).
  - Impact row: `IntroImpactItem.body` is now optional (heading-only items render fine); a new `IMPACT_SLOTS=3` always lays out 3 columns via filler divs so a 2-item row's columns still line up with a 3-item row's; `impactItemHeight` prop overrides the shared 108px per row (Path Analysis uses 64px, its items are all heading-only). Impact body color changed from `tertiary`→`secondary` (rendered via raw `Title`+`Block` now, not `TitleBlock`, since `TitleBlock` hardcodes `tertiary`).
  - Meta row's "Company" value is now a recolored brand-logo mark (new `CompanyLogo.tsx`) instead of plain text — see the dedicated entry below, still open (logo heights need per-instance balancing).
  - `caseStudyIntro.ts` (shared type) gained: `description: ReactNode` (was `string` — Session Replay links a phrase inline to Heap's product page), `IntroImpactItem.body?` (optional), `companyLogo: CompanyLogo` (new field).
  - **DHM's prototype moved:** `/work/data-health-monitor` (bare slug) is now free for the eventual case study page — the live prototype screens moved to `/work/data-health-monitor/prototype` (+ `/prototype/domain`), relative `../../../design-systems` imports converted to `@/` alias in the process.
  - **Branch renamed:** `sw-observability-rewrite` → `wip` (this doc's project-doc-set references to the old name are historical, not stale — the branch itself is gone, code is unaffected).
  - **Next session starts here:** full visual pass on all 4 rows — corner rounding (should read as floating rounded cards for DHM/Session Replay/Path Analysis, flush-square-bottom for SW Obs only), DHM's embed height/crop (frozen at the top of a real dashboard page, `pointer-events:none` so it can't be scrolled/clicked), both videos' fit + autoplay-after-settle timing, impact-row filler alignment across all 4 rows at the ≤620px stacked tier (untested there).
- **Tooltip + CompanyLogo built this session (new components), only partially wired — logo heights explicitly flagged as wrong.** `components/Tooltip.tsx` (Figma node 1102:144, "Portfolio Cleaning" file) — hover card portaled to `document.body`, `top-start` placement only (+ flip to `bottom-start` near the viewport top), 250ms close delay via new token `--motion-delay-tooltip-close` (this system's first motion token; XOPS's own `Tooltip.tsx` got the equivalent fix — its previously-hardcoded `250` in `setTimeout` now reads `CLOSE_DELAY_MS`, mirroring its own new `--xops-motion-delay-tooltip-close` — the two systems' tokens stay separate, not shared). `components/CompanyLogo.tsx` — recolors the XOPS/Heap/Auryc brand SVGs (`public/SVG/{XOPS,Heap,Auryc}Logo.svg`, renamed on intake to drop spaces) to a flat color via CSS mask-image (same technique as XOPS's `Icon.tsx`), default height `--text-body-xs-lh` with an optional per-instance `height` override, width always from the logo's own real `aspect-ratio`. `TitleBlock`'s `title` prop widened `string`→`ReactNode` to fit the "Company" + info-icon layout (new `.companyLabel` class, 6px gap — literal px, doesn't match any spacing-scale step, user-specified exact value).
  - **User feedback, not yet acted on:** "the logos need individual tweaking they can all have the same height it doesn't feel balanced" — `CompanyLogo`'s `height` override prop exists for exactly this, but no per-case-study values have been set yet (none of the 3 `introContent.ts` files pass a `height` prop). **Next session starts here:** get the user's actual per-logo height numbers (or a live back-and-forth adjusting them) and wire `height={...}` into each case study's `companyLogo` entry.

...
| 13–14 | Overview Prototypes 1 & 2 | superseded — see "Hotspot Annotation System" below | `section.overview-prototype-1`, `section.overview-prototype-2` | — | 🔶 both prototypes now live-embed hotspot experiences (Proto 1: 7 Assumption hotspots; Proto 2: 7 Decision hotspots, blind pass); touch support still open. Sidebar redesign in progress (countdown/button/shortcuts + Timeline restyle, Figma nodes 941:774/941:812/941:810) — direction is to eventually extract a shared reusable "Player" component for the other hotspot experiences (All Software, Profile, Final Lifecycle Timeline, currently on the older `HotspotOverlay` scroll-pin mechanism), deliberately deferred until this instance is built + visually confirmed first; see progress.md Resume Context |
